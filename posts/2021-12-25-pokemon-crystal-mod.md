...
<label><input type="checkbox" id="rebattle-trainers"> Rebattle trainers</label> (Talk to already beaten trainer to fight them again.)<br>
<label><input type="checkbox" id="infinite-tms"> Infinite TMs</label> (TMs are not consumed when used.)<br>
<label><input type="checkbox" id="forget-hms"> Forgettable HMs</label> (HMs can be forgotten by learning new moves.)<br>
<label><input type="checkbox" id="boost-exp"> Boosted Exp.</label> (Pokémon always gain boosted experience points.)<br>
<label><input type="checkbox" id="reset-rtc"> Passwordless RTC Reset</label> (Change the real-time clock with any password.)<br>
...
function patchROM() {
	...
	reader.onload = () => {
		...
		if (!success) {
			alert("The patterns were not found, some patches could not be applied!");
		}
		//else {
